...
import cache, { bindingsCacheToAdd, bindingsFuncsToAdd } from '../logic/cache';
// TODO: add support for multiline contexts and values
// TODO: support single line comments using "//"
const snapshots = {}; // cache for generated StructureDefinition snapshots
...
const funcs: PreCompilerFunctions = {
  ...
  getDescendantElementDef: async (fhirType: string, path: string): Promise<any> => {
    // logger.info('getDescendantElementDef called');
    // logger.info({ fhirType, path });
    if (fhirType === 'BackboneElement' && !['id', 'extension', 'modifierExtension'].includes(path)) return undefined;
    return await funcs.getDescendantElementDefExpr.evaluate({}, { info: fumeFuncs.logInfo, fhirType, path, getSnapshot, getStructureDefinition: fumeFuncs.getStructureDefinition, getChildOfBaseType: funcs.getDescendantElementDef, initCap: initCapOnce, endsWith });
  },
  buildSnapshotExpr: jsonata(`
  /* this script builds a snapshot of a StructureDefinition */
  /* TODO: fix order of slice block's children */
  ...
  buildSnapshot: async (rootType: string, path: string): Promise<any> => await funcs.buildSnapshotExpr.evaluate({}, { info: fumeFuncs.logInfo, rootType, path, getStructureDefinition: fumeFuncs.getStructureDefinition, initCap: initCapOnce, startsWith, endsWith, getChildOfBaseType: funcs.getDescendantElementDef }),
  ...
  getElementDefinition: async (rootType: string, path: string): Promise<any> => await funcs.getElementDefinitionExpr.evaluate({}, { info: fumeFuncs.logInfo, throwError: thrower.throwParseError, getSnapshot, rootType, path, getDescendantElementDef: funcs.getDescendantElementDef, endsWith, initCap: initCapOnce }),
  ...
  getMandatoriesOfElement: async (structId: string, relativePath: string, structureFunction: Function): Promise<any> => await funcs.getMandatoriesOfElementExpr.evaluate({}, { info: fumeFuncs.logInfo, structId, relativePath, getMandatoriesOfElement: funcs.getMandatoriesOfElement, structureFunction, getSnapshot, startsWith, initCap: initCapOnce })
};
...
const toJsonataString = async (inExpr: string): Promise<string | undefined> => {
  ...
  const lineParser = async (line: string, index: number, allLines: string[]) => {
    // logger.info('lineParser called');
    // logger.info({ line, index, allLines });
    ...
    // logger.info({ expressionHasFlash });
    ...
    if (isInstanceOfDeclaration) {
      ...
      const indent: string = substringBefore(line, 'InstanceOf:');
    ...
    // logger.info({ endOfFlashBlock, 'allLines.length': allLines.length, index });
    if (expressionHasFlash && (endOfFlashBlock >= 0 || (flashBlockHanging && index === (allLines.length - 1)))) {
      // logger.info('endOfFlashBlock >= 0 || (expressionHasFlash && index === (allLines.length - 1))');
      // logger.info({ endOfFlashBlock, expressionHasFlash, index, 'allLines.length': allLines.length });
      ...
      // logger.info({ hangingBracketDiff });
      // eslint-disable-next-line @typescript-eslint/restrict-plus-operands
      ...
      if (eDef) {
        ...
        if (isPoly) {
          ...
          if (eDef?.type.length === 1) {
            ...
            // logger.info('constructLine, select from types using comparison function');
        ...
        // logger.info('constructLine, jsonPrimitiveProfile: string = chosenTypeEntry?.extension?.filter');
        ...
        // $constructMandatoryLine($prefix, $rootStructDef, $currentPath, $);
        // const mandatoryLine = await constructMandatoryLine('', rootStructDef, currentFshPath);
        // logger.info({ mandatoryLine });
  ...
  try {
    ...
    // logger.info('got expression parsed');
    // logger.info({ parsed });
    if (!expressionHasFlash) {
      ...
      parsed = parsed.includes('finalize') ? parsed : `${parsed}$__flashInstance := $__finalize($__flashInstance);`
...
const transform = async (input, expression: string, bindingsFuncsToAdd = [], bindingsCacheToAdd = []) => {
  ...
  try {
    ...
    const expr = await addFumeFuncs.compiledExpression(expression);

    let bindings: Record<string, Function | Record<string, any>> = {};

    // bind all mappings from cache
    const mappingIds = Object.keys(cache.mappingCacheCompiled);
    if (mappingIds) {
      mappingIds.forEach((mappingId) => {
        bindings[mappingId] = cache.mappingCacheCompiled[mappingId].function;
      });
    }
    bindingsCacheToAdd.forEach(c => {
      bindings[c] = cache[c];
    });

    // bind functions
    bindingsFuncsToAdd.forEach(f => {
      bindings[f] = addFumeFuncs[f];
    });
    ...
    bindings.registerTable = addFumeFuncs.registerTable;
    bindings.initCap = stringFuncs.initCap;
    bindings.isEmpty = fumeFuncs.isEmpty;
    ...
    bindings.warning = fumeFuncs.logWarn;
    bindings.info = fumeFuncs.logInfo;
    ...
    bindings.getDescendantElementDef = funcs.getDescendantElementDef;
    bindings.fhirCacheIndex = cache.fhirCacheIndex;
    bindings.getSnapshot = getSnapshot;
    bindings.getStructureDefinition = fumeFuncs.getStructureDefinition;
    bindings.v2dictionary = HL7Dictionary.definitions;
    bindings.v2codeLookup = v2.v2codeLookup;
    bindings.v2tableUrl = v2.v2tableUrl;
    bindings.toJsonataString = toJsonataString;
...
const mappingToJsFunction = (mapping) => {
  // fork: os
  return async (input) => {
    const res = await transform(input, mapping, bindingsFuncsToAdd, bindingsCacheToAdd);
    return res;
  };
};
...
const cacheMapping = (mappingId: string, mappingExpr: string) => {
  ...
  cache.mappingCacheCompiled[mappingId] = cacheEntry;
};
